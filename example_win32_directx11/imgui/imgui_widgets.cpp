...
inline ImVec4 checkbox_gradient_start = ImColor(101, 14, 156, 255);
inline ImVec4 checkbox_gradient_end = ImColor(75, 50, 237, 255);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.gradient_color0 = ImLerp(it_anim->second.gradient_color0, *v ? checkbox_gradient_start : hovered ? ImColor(0, 0, 0, 0) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.gradient_color1 = ImLerp(it_anim->second.gradient_color1, *v ? checkbox_gradient_end : hovered ? ImColor(0, 0, 0, 0) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.checkmark_color = ImLerp(it_anim->second.checkmark_color, *v ? checkbox_mark : hovered ? ImColor(0, 0, 0, 0) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 9.f);
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.y > grab_bb.Min.y) {
        const ImVec2 grab_pos = ImVec2((grab_bb.Min.x + grab_bb.Max.x) * 0.5f, (grab_bb.Min.y + grab_bb.Max.y) * 0.5f);
        const float grab_size = ImMin(grab_bb.GetWidth(), grab_bb.GetHeight());
        const ImU32 grab_col = GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
        window->DrawList->AddTriangleFilled(grab_pos, ImVec2(grab_pos.x - grab_size, grab_pos.y - grab_size * 0.5f), ImVec2(grab_pos.x - grab_size, grab_pos.y + grab_size * 0.5f), grab_col);
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    // For the vertical slider we allow centered text to overlap the frame padding
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y -30), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.0f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y -30), label);
