...
namespace keybind
{
	...
	{
		static bool b_get = false;
		static std::string sz_text = xorstr("None");
		...
		if (b_get)
		{
			...
			sz_text = xorstr("") + get_key_name_by_id(key);
	}

	void key_bind1(int& key1, int width, int height)
	{
		static bool b_get1 = false;
		static std::string sz_text1 = xorstr("None");

		if (ImGui::Button(sz_text1.c_str(), ImVec2(static_cast<float>(width), static_cast<float>(height))))
			b_get1 = true;
		...
		else if (!b_get1 && key1 == 0)
			sz_text1 = xorstr("None");
		else if (!b_get1 && key1 != 0)
			sz_text1 = xorstr("") + get_key_name_by_id(key1);
	}
}
